...
// 1. Single Chat Item
const ChatItem = memo(({ chat, isActive, isOnline, currentUser }) => {
    ...
    const getPreviewText = () => {
        if (isBlockedByMe) return <span className="text-red-400 italic flex items-center gap-1"><Ban size={10} /> You blocked them</span>;
        if (isBlockedByPartner) return <span className="text-muted italic flex items-center gap-1">User unavailable</span>;

        return (
            <>
                {isMe && <span className="text-xs opacity-70 font-normal">You:</span>}
                {lastMsg?.message_type === "image" ? "📷 Photo" :
                    lastMsg?.message_type === "audio" ? "🎤 Voice" :
                        lastMsg?.message_type === "story_reply" ? "🔄 Re: Story" :
                            lastMsg?.message_type === "shared_post" ? "📌 Shared Post" :
            ...
            <motion.div
                ...
                className={`relative flex items-center gap-3 px-3 py-3.5 rounded-2xl transition-all duration-200 group border cursor-pointer
                    ${isActive
                        ...
                        : isUnread
                            ? "bg-main border-l-4 border-l-primary shadow-sm"
            ...
            >
                ...
                <div className="relative shrink-0">
                    ...
                    {isOnline && !isConnectionSevered && (
                        <span className="absolute bottom-1.5 right-0 z-10 w-3 h-3 bg-green-500 border-2 border-surface rounded-full shadow-sm"></span>
                    )}
                    {isBlockedByMe && (
                        <span className="absolute bottom-0 right-0 z-10 p-0.5 bg-surface rounded-full border border-adaptive">
                ...
                <div className="flex-1 min-h-0">
                    <div className="flex justify-between items-center mb-0.5">
                        ...
                        <span className={`text-[10px] ${isActive || isUnread ? "text-primary font-bold" : "text-muted group-hover:text-muted/80"}`}>
                            {lastMsg?.createdAt ? formatDistanceToNowStrict(new Date(lastMsg.createdAt)) : ""}
                        </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        {unreadCount > 0 && !isConnectionSevered && (
                            <div className="w-5 h-5 bg-primary text-white text-[10px] flex items-center justify-center rounded-full shadow-lg shadow-primary/40 animate-pulse ml-2 font-bold">
                                {unreadCount}
                            </div>
                        )}
                    </div>
                ...
                {isActive && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-1 bg-primary rounded-r-full shadow-[0_0_10px_var(--color-primary)]"></div>
                )}
            </motion.div>
        </Link>
    );
...
const RecentMessages = () => {
    ...
    return (
        <motion.div
            className="w-full flex flex-col text-sm bg-surface/60 backdrop-blur-xl rounded-2xl shadow-sm border border-black/5 dark:border-white/5 overflow-hidden h-full"
            ...
            <div className="p-4 border-b border-black/5 dark:border-white/5 bg-surface/80 backdrop-blur-md">
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-main border border-black/5 dark:border-white/10 rounded-xl py-2 pl-9 pr-3 text-xs text-content focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all placeholder-muted"
                    />
                </div>
            </div>
